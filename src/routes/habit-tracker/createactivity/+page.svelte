<script lang="ts">
	...
	function selectSuggestion(suggestion: UserActivity) {
		const { name, points, activity_id } = suggestion;
		activityName = name;
...
{#if form}
	...
	{#if form.success}
		<div class="flex gap-2 justify-center items-center">
			...
			<a href="/habit-tracker/dailyactivitylist" class="text-blue-500 underline"
...
<form action="?/setActivity" method="POST" class="flex flex-col">
	...
	<Button
		disabled={!userActivities.some((activity) => activity.name === activityName)}
		type="submit"
		class="bg-blue-500 mt-5 mx-auto ">Set the activity</Button
	>
</form>
